### 由于官方的小程序进行了接口和地址上的一些“升级”，目前已无法通过现有的方式进行二维码登录便捷请求小程序获取数据了；小程序降将token与appid做了强绑定校验，通用OpenID已无法再通过小程序的验证了，本仓库将归档，后续不再进行更新，目前网页只支持手动绑定Cookie登录，但已支持战绩的持续记录并统计，但不再开源，并非跑路，而是真没招了
